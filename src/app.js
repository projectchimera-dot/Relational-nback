...
function renderStimulus(visible) {
  ...
  else draw3DStimulus(cube, currentTrial(), { yaw: state.config.mode === '3d-static' ? 0.72 : getRotation(), pitch: -0.43, visible });
}

function animateCube() {
  cancelAnimationFrame(animationFrame);
  const loop = () => {
    if (state?.running && state.config.mode === '3d-rotating' && !state.paused) draw3DStimulus(cube, currentTrial(), { yaw: getRotation(), pitch: -0.43, visible: state.visible });
