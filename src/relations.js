...
export function viableNumberPrompts(current, maxAbsDelta = 4) {
  const out = [];
  if (current > 0) out.push({ type: 'BEFORE' });
  if (current < 9) out.push({ type: 'AFTER' });
  for (let delta = -maxAbsDelta; delta <= maxAbsDelta; delta += 1) {
    const result = current + delta;
    if (result >= 0 && result <= 9) out.push({ type: 'DELTA', delta });
  }
  return out;
}
...
export function viableShapePrompts(currentShape) {
  const current = SHAPE_SIDES[currentShape];
  if (!current) return [];
  const sideValues = Object.keys(SHAPE_BY_SIDES).map(Number);
  const out = [];
...
export function applyShapePrompt(currentShape, prompt) {
  const current = SHAPE_SIDES[currentShape];
  if (!current || !prompt) return null;
  const values = Object.keys(SHAPE_BY_SIDES).map(Number).sort((a, b) => a - b);
