...
export function createRelationVisual(relationId, options = {}, rng = Math.random) {
  ...
  const nodes = relationNodes(relationId, { moreShape, lessShape, flipDiagonal });
  return {
    semantic: relationId,
...
function relationNodes(relationId, { moreShape, lessShape, flipDiagonal }) {
  switch (relationId) {
    case 'SAME':
      return [
    ...
    case 'LESS':
      return [
        { shape: lessShape, cx: 38, cy: 60, size: SMALL_SIZE },
        { shape: moreShape, cx: 83, cy: 60, size: BIG_SIZE },
      ];
    case 'LEADS_TO': {
