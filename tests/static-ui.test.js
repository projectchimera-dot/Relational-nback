...
test('game HUD exposes current per-trial N directly above the grid and responses follow the grid', async () => {
  const html = await read('index.html');
  const hud = html.indexOf('id="trial-n-display"');
  const grid = html.indexOf('id="grid-2d"');
  const responses = html.indexOf('class="response-deck"');
  assert.ok(hud >= 0, 'current per-trial N display should exist');
  assert.ok(hud < grid, 'current N display should appear before the grid');
  assert.ok(grid < responses, 'response deck should appear after the grid');
  const app = await read('src/app.js');
  assert.match(app, /trial-n-display/);
  assert.match(app, /trial\.nBack/);
});

test('wiki explicitly explains side-count relation rules and left-right irrelevance', async () => {
  const wiki = await read('src/wiki.js');
  assert.match(wiki, /more-sided shape is outside/i);
  assert.match(wiki, /more-sided shape is inside/i);
  assert.match(wiki, /more-sided shape is larger/i);
  assert.match(wiki, /more-sided shape is smaller/i);
  assert.match(wiki, /more-sided shape is above/i);
