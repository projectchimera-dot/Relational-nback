...
test('greater relation does not encode meaning by left versus right placement', () => {
  const makeRng = (values) => {
    let index = 0;
    return () => values[index++] ?? values.at(-1) ?? 0;
  };
  const left = createRelationVisual('GREATER', { enabledShapes: ['CIRCLE', 'HEXAGON'] }, makeRng([0, 0]));
  const right = createRelationVisual('GREATER', { enabledShapes: ['CIRCLE', 'HEXAGON'] }, makeRng([0, 0.99]));
  const leftMore = left.nodes.find((node) => node.shape === 'HEXAGON');
  const rightMore = right.nodes.find((node) => node.shape === 'HEXAGON');
  assert.notEqual(leftMore.cx, rightMore.cx, 'more-sided shape should be able to appear on either side');
  assert.ok(leftMore.size > left.nodes.find((node) => node.shape === 'CIRCLE').size);
  assert.ok(rightMore.size > right.nodes.find((node) => node.shape === 'CIRCLE').size);
});
